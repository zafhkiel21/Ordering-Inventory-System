...
Public Class Records
    ...
    Sub loadingDatagridView()
        Try
            ...
            conn.Open()
            Dim cmd As New OleDb.OleDbCommand("Select * from Stocks", conn)
            dr = cmd.ExecuteReader
            While dr.Read
                DataGridView1.Rows.Add(dr.Item("ID"), dr.Item("Orgshirt"), dr.Item("Date"))
            End While
            dr.Close()
        Catch ex As Exception
            MsgBox(ex.Message)
    ...
    Sub save()
        Try
            ...
            If MsgBox("Are you want to add this?", vbQuestion + vbYesNo) = vbYes Then
                Dim sql As String = "Insert into Stocks (Orgshirt) values (@Orgshirt)"
                Dim cmd As New OleDb.OleDbCommand(sql, conn)
                cmd.Parameters.Clear()
                cmd.Parameters.AddWithValue("@Orgshirt", txt_rcv.Text)
